...
個人靜態網站。目前有六個頁面，都放在不容易被猜到的隨機路徑底下，並加上
`noindex` 避免被搜尋引擎收錄。
...
| 應用 | 照明、顯示、雷射、車用、健康照明、園藝、近紅外感測、紫外光療與消毒、夜光與應力發光、醫療影像（閃爍體）、生醫與上轉換，共 11 項 |
| 全球版圖 | 日本、美國、歐洲、中國、韓國、台灣六區的企業與研究團隊，加上「稀土供應鏈」與「學術與會議」兩個專題分頁；市場規模九家估計並列；專利地圖 |
| 最新動態 | 2024–2026 年的產品、論文、專利授權、法規、市場、併購事件共 80 餘則，每則附「為什麼重要」與信心等級，可依年份、主題、地區篩選 |
| 研究前沿 | 窄頻紅粉、KSF 抗濕、雷射陶瓷、Micro LED 色轉換、AI 材料發現等九個方向 |
| 常見問題 | 毒性、稀土、光衰與壽命、閃爍、怎麼選燈、螢光粉對量子點、藍光、回收、台灣、AI 找材料… |
| 名詞解釋 | 80 多條術語，可搜尋 |
| 來源說明 | 整理方法、已知限制、引用來源清單（由產生器自動彙整） |

### 資料怎麼來的

2026 年 9 月以網路檢索一次性整理，分兩輪：第一輪分六個主題各自蒐集附來源的
事實並由獨立步驟查核；第二輪另開七個研究工作階段（產業與供應鏈、學術版圖與
AI、2024–2026 動態、應用、台灣與東亞、夜光／閃爍體／近紅外），並對站內 181 條
敘述逐條查核（149 條確認、24 條修正、8 條查無來源、0 條推翻），修正已全部併入。
每則資料都附上當時可查到的公開來源連結；市場數字各家不一致時並列範圍而不下結論。
...
- **題型與配分比照正式學測**：單選 6 題、多選 6 題、選填 5 題（各 5 分），
  混合題一個題組 3 小題（15 分，含一題非選擇題）。
- **範圍**：108 課綱數學 1、2、3A、4A，共 15 個單元；每一單元附「預測熱度」
  （編者依課綱份量與近年命題習慣估計）與這份卷子裡對應的題號。
- **每題內容**：題幹、作答區、核心意義一句話、逐步詳解、「為什麼預測會考」、
  以及專屬動畫。多選題採學測的部分給分（錯 1 個選項 3 分、錯 2 個 1 分），
...
沒有用 KaTeX / MathJax：題目文字採用一套很小的類 TeX 語法（`$...$`、`^`、`_`、
`\frac`、`\sqrt`、`\vec`、`\b{13}` 畫選填圈號…），由 `app.js` 的 `M()` 轉成
HTML＋CSS。好處是完全離線、載入快、不受 CDN 影響；限制是只支援學測題目會用到
的那些符號，新增題目時若用了不支援的指令，會原樣顯示出來，很容易發現。
