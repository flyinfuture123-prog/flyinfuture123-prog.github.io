...
| 全書導讀 | 依原書章節整理，可展開閱讀 |
| 來源與缺頁 | 說明哪幾頁已取得、哪幾頁還沒有 |

### 資料怎麼來的

原始檔為 CamScanner 掃描的 PDF（53.7 MB），存放於 Google Drive。因為
連接器的下載上限是 10 MB，檔案被切成七份分批取得，再依印在頁面上的頁碼
還原順序：

- 第 1–15、78–81 頁 — 用 PyMuPDF 轉成頁面圖片後逐頁判讀，圖表與手寫內容都保留
- 第 27–36、46–77 頁 — 由 Google Drive 的 OCR 取得文字，圖片僅存圖說

### 已知限制

- **內容不完整。** 第 16–26 頁（第三章 資料庫檢索）、37–45 頁（4-4 後半與
  4-5）、82–96 頁（7-3 後半、第八章、作者簡歷）尚未取得，頁面上以「待補」
  標示，補進掃描頁後重跑產生器即可。
- OCR 段落的化學式與表格數值可能有辨識誤差，已知原書表一、表二的化學式
  在 OCR 後多處失真，頁面上只保留欄位結構與摘要說明，未逐格照抄。
